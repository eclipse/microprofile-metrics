...
Metrics can be added to or retrieved from the registry either using the <<annotations, Metrics Annotations>> or using the `MetricRegistry` object directly.
...
* 406 if the HTTP Accept Header in the request header cannot be handled by the server.
...
The metadata will be included inline in the Prometheus text format, specified by it. Unlike the JSON format, the text format does not support OPTIONS requests.
...
The Prometheus text format adheres to using "base units" when creating the HTTP response. Due to the different context of each metric type, certain metrics' values must be converted to the respective "base unit" when responding to Prometheus requests.
...
- If the metric's metadata contains a known unit defined in <<metric_units>>, the Prometheus value should be scaled to the _base unit_ of the respective family. The name of the base unit is appended to the metric name delimited by underscores (`_`).
- If the `unit` is specified and is not defined in <<metric_units>>, the value is not scaled but the `unit` is still appended to the metric name delimited by underscores (`_`).
