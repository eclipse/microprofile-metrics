...
Default is `false`.
The implementation must throw an `IllegalArgumentException` in case an attempt is made to register a metric multiple times where not all instances
are marked as reusable.
See also <<reusing_of_metrics>>.
