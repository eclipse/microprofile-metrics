...
* Changes since 1.0
** Added integration scope
** Added sub-scopes to application and integration
...
The following sets of sub-resource (scopes) are exposed.
...
* integration: a place for other MicroProfile specs to expose their data
...
Some of the scopes allow to provide sub-scopes to separate metrics with the same name from possibly different
sources. When sub-scopes are used, they serve as an intermediate layer below the main scope.
Sub-scopes are optional and may be mixed with metrics at scope level.


.Example of mixing metrics without sub-scope with sub-scoped data
----
application:application_count # <1>
application:myShop:checkout_count # <2>
application:myShop:checkout_amount # <3>
application:otherApp:checkout_count # <4>
----
<1> a metric without sub-scope
<2> metric in myShop sub-scope
<3> metric in myShop sub-scope
<4> metric in otherApp sub-scope

TODO: how do we make sure that metric names directly below scope do not clash with sub-scope names?

TODO: should sub-scopes only be optional for applications and mandatory for integration scope?
...
Sub-scopes are allowed to expose metrics of different applications that are deployed into one server.
...
TODO: shorter name of scope like 'int' ?

This scope allows for other MicroProfile specifications or technologies to expose their data
in well-known places. 

TODO: do we also want to expose non-MP metrics here like Servlet stats?

NOTE: An example could be Fault Tolerance where it is possible to define how often a call to a remote system is 
automatically re-done before it then possibly fails. It is possible that on each call it takes 3-4 retries, so that
the maximum number of retires is never reached. Nevertheless there is an issue, as there are always retries necessary.
Exposing the retry-numbers help operators to find this issue.   

Integrations can use the <<app-metrics-api>> to expose their data in the respective scope

Sub-scopes are required to expose metrics of different MicroProfile specifications. The sub-scopes
serve as name spaces. The next table shows the technology and the respective name of the sub-scope.


.Defined sub-scopes
[cols="1,3"]
|===
|Name of sub-scope |Technology
...
Scopes that allow for sub-scopes also have sub-registries, one per sub-scope.
...
| `/metrics/<scope>/<sub-scope>/<metric_name>` | GET | JSON, Prometheus | Returns the metric that matches the metric name for the respective scope and sub-scope
...
| `/metrics/<scope>/<sub-scope>/<metric_name>` | OPTIONS | JSON | Returns the metric's metadata that matches the metric name for the respective scope and sub-scope
|===
...
In case `/metrics` is requested, then the data for the scopes and sub-scopes are wrapped in the scope name:
...
* Sub-scopes, if present, are inserted between scope and metric and end with a colon (`:`)
...
[[app-metrics-api]]
== Application and Integration Metrics Programming Model

MicroProfile Metrics provides a way to register Application-specific metrics to allow applications to expose metrics in
the _application_ scope. 

Likewise metrics of integrations can be exposed under the _integration_ scope (see <<scopes>> for the description of scopes).
...
There is one shared singleton of the `MetricRegistry` per scope (_application_, _base_, _integration_ and _vendor_).
When metrics are registered using annotations, the metrics are registered in the _application_ `MetricRegistry` 
(and thus the _application_ scope).

When injected, the `@RegistryType` is used as a qualifier to selectively inject either the `APPLICATION`, `BASE`, 
`INTEGRATION` or `VENDOR` registry.
...
The implementation must produce the _application_ `MetricRegistry` when no `RegistryType` is provided (`@Default`) 
...
If the application wishes to use a sub-scope, it can apply the `@SubRegistry` annotation to set a deployment-wide
sub-registry:

.Example with sub-scope
[source, java]
----
@SubRegistry(name="myApp")
...
The sub-registry can also be provided in a programmatic way:

[source, java]
----
@Inject
@RegistryType(type=MetricRegistry.Type.APPLICATION, subtype="myApp")
MetricRegistry metricRegistry;
----
...
==== Integration Metric Registry
The implementation must produce the _integration_ `MetricRegistry` when the `RegistryType` is `INTEGRATION`. 
The _integration_ `MetricRegistry` must contain any integration specific metrics in respective sub-registries.
Integrations need to always provide their sub-scope.

.Example of the application injecting the Fault-Tolerance integration registry
[source, java]
----
@Inject
@RegistryType(type=MetricRegistry.Type.INTEGRATION,subtype="ft")
MetricRegistry vendorRegistry;
----
...
public class MetricRegistryFactory {
    ...
    @Produces
    @RegistryType(type = Type.INTEGRATION)
    public static MetricRegistry getIntegrationRegistry() {
        // Returns the static instance of the Integration MetricRegistry
        [...]
    }
