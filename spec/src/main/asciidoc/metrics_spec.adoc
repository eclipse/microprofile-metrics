...
When a field of type Meter, Timer, Counter or Histogram is annotated, the implementation must provide the registered metric to the application. The implementation must register the metric for the field using the <<annotated-naming-convention>> if it does not already exist, otherwise it will retrieve the metric from the registry and produce it to the application.
