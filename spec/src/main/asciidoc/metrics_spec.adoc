...
** meter: a metric which measures mean throughput and one-, five-, and fifteen-minute exponentially-weighted moving average throughput.
** histogram: a metric which calculates the distribution of a value.
** timer: A timer metric which aggregates timing durations and provides duration statistics, plus throughput statistics.
...
|@Counted | M, C, T | Denotes a counter | MetricUnit.NONE
|@Gauge   | M, F | Denotes a gauge.  | _none_ Must be supplied by the user
|@Metered | M, C, T  | Denotes a meter which counts the invocations of the annotated object. | MetricUnit.NANOSECOND
...
|@Timed   | M, C, T | An annotation for marking a constructor or method of an annotated object as timed.
    The metric of type Timer records the time it took to invoke the annotated object. | MetricUnit.NONE
|===
...
|@RegistryType| Qualifies the type of Metric Registry to inject.|  _application_ (scope)
|===
...
`String name`:: Optional. Sets the name of the metric. If not explicitly given it is determined from the annotated object and the
fully qualified name of the surrounding class, unless `absolute` is true.
`boolean absolute`:: If `true` use the given name as an absolute name of the metric. If `false`, use the given name relative to
the annotated class. Default value is `false`.
...
===== TYPE
When a type/class is annotated, the implementation must register a counter for each of the constructor and methods using the <<annotated-naming-convention>>. The counters are incremented and/or decremented according to the `monotonic` field when the corresponding constructor/method is invoked.
...
===== TYPE
When a type/class is annotated, the implementation must register a meter for each of the constructor and methods using the <<annotated-naming-convention>>. The meters are marked each time the corresponding constructor/method is invoked.
...
===== TYPE
When a type/class is annotated, the implementation must register a timer for each of the constructor and methods using the <<annotated-naming-convention>>. Each time a constructor/method is invoked, the execution will be timed with the corresponding timer.
