...
* Changes in 2.3
** Introduced the `@NamedMetadata` annotation that allows declaring a shared metadata definition that can be referenced by regular metric annotations.
** The `@Gauge` annotation no longer requires a value for the `unit`. It now defaults to `MetricUnits.NONE`.
