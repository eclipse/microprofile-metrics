...
*** Former non-monotonic counters are now <<app-metrics-model#ConcurrentGaugeDef,`@ConcurrentGauge`>> and also in the output reported as gauges.
...
** icon:bolt[role="red"]  `Metadata` is now immutable and built via a <<app-metrics-model#pgm-metadata,`MetadataBuilder`>>.
...
** icon:bolt[role="red"] <<rest_endpoints#json-format-def,JSON output format>> for GET requests now appends tags along with the metric in `metricName;tag=value;tag=value` format.
