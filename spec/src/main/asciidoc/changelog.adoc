...
*** Changed `Timer.update(long duration, java.util.concurrent.TimeUnit)` to `Timer.update(java.time.Duration duration)`
*** Added `Timer.getSum()` which returns `java.time.Duration` (https://github.com/eclipse/microprofile-metrics/issues/597[#597])
...
** Updated `SimpleTimer` to rename `SimpleTimer.getElapsedTime()` to `SimpleTimer.getSum()` (https://github.com/eclipse/microprofile-metrics/issues/597[#597])
...
*** This value is exposed as a `sum` value in JSON format and is exposed as the `<metric_name>_<units>_sum`under the `summary` type in OpenMetrics format (https://github.com/eclipse/microprofile-metrics/issues/597[#597])
...
** Histogram and Timer now expose the total sum of recorded values as a `sum` value (https://github.com/eclipse/microprofile-metrics/issues/597[#597])
