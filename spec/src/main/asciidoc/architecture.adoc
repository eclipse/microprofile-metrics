...
* OpenMetrics text format - default response format when the HTTP Accept header does not match any more specific
media type like `application/json`.
+
NOTE: Future versions may allow for more export formats that are triggered by their specific media type. The OpenMetrics
