...
Data is exposed via REST over HTTP under the `/metrics` base path in different data formats.
There are two formats, described below that every implementation must provide.
On top it is possible to provide more formats via <<exporter-spi,a provider interface>>.

The formats for `GET` requests are:
...
All metric exporters implement the `org.eclipse.microprofile.metrics.spi.MetricExporter` interface, which defines
methods to export single metrics, a scope and all metrics.
Those methods are expected to return a fully formatted String that can be sent as-is to the requesting client.
It also provides the following methods to identify the supported media types and Http verbs:

.Excerpt of MetricExporter
[source, java]
----
  /**
...
On top it defines a method to pass in the registries as a Map keyed by Scope:
...
Implementations must determine the desired exporter by matching the media type and http verb with the incoming request.
The implementation must return a "406 Not acceptable" code if such a combination is not provided.

The implementation uses the exporter with the highest priority if two or more exporters support the
same media type and http verb.

The built-in converters have a priority of 1.
The implementation must register the default exporters first and reject exporters that have the same value of
media type/http verb/priority than previously registered ones.


==== Service Loader

Implementations must allow to load exporters via the `ServiceLoader` interface.

Register your implementation in a resource file `/META-INF/services/org.eclipse.microprofile.metrics.spi.MetricExporter`
by including the fully-qualified class name of the exporter implementation/s in the file.

The default exporters may be provided via service loader or can be bundled with the core implementation.
