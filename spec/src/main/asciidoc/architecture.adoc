...
The ParallelCounter keeps a high-water mark for a (server-wide) configurable interval.
...
=== Duties of the runtime

The following sections lists duties of the runtime.
This spec lists further requirements in other sections.

==== ParallelCounters

ParallelCounters keep a high water mark for a configurable interval (global value for the server).
The default value is one minute.

The interval must be configurable via MicroProfile Config under the
key `mp.metrics.high_water__mark_interval_seconds`.
Allowed values are in the range of 30s to 10 minutes.

The runtime must ensure that the high water mark is reset to 0 after the interval has elapsed.
