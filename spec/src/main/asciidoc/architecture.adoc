...
All metric types except gauges support being declared using annotations in CDI beans which use such scope that there will be multiple instances of the bean created 
over the lifecycle of the application. If multiple instances of the bean with a metric exist, usages of the metric will be merged together, so for example,
