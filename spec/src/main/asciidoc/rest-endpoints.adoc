...
The property `mp.metrics.distribution.timer.slo` applies to timer metrics and the property `mp.metrics.distribution.histogram.buckets` applies to histograms. These two properties follows these rules unless explicitly describing a specific property:
...
----
//alpha.histogram will publish histogram-buckets with maximum values of 10.0 and 50.0 and 100.0 while the beta.histogram will publish histogram buckets with maximum values of 30.0, 50 and 123.
...
----
//alpha.timer will publish histogram-buckets with maximum values of 500 milliseconds, 2 second, and 3 minutes while the beta.histogram will publish histogram buckets with maximum values of 10 seconds, 2 minutes and 5 hours. Timer metrics that do not match will not have any histogram buckets.
