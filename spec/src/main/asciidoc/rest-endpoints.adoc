...
In addition the saved value of the minute, which can be retrieved via `getRecentPeak()` are
supplied by attaching `.recentPeak` as key.
...
HitCounters get an additional suffix `_total` unless the metric name already ends in `_total`.

.Example Prometheus text format for a HitCounter.
[source, ruby]
----
# TYPE application:visitors_total counter
# HELP application:visitors_total The number of unique visitors
application:visitors_total 80
----

===== ParallelCounter

ParallelCounters get a suffix of `_activecount`.
On top for each ParallelCounter an additional value line for the
high water mark is created by appending `_recent_peak`.

.Example Prometheus text format for a ParallelCounter.
[source, ruby]
----
# TYPE application:visitors_activecount gauge
# HELP application:visitors_activecount The number of unique visitors
application:visitors_activecount 80   <1>
# TYPE application:visitors_activecount_recent_peak gauge
application:visitors_activecount_recent_peak 150  <2>
----
