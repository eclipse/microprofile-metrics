...
** `mp.metrics.distribution.timer.buckets` accepts a comma separated list of integers with an appended time unit (valid time units are `ms` for milliseconds, `s` for seconds, `m` for minutes and `h` for hours). Decimal values will not be accepted. Values with no time unit default to milliseconds. _(See <<timer-buckets-sample,examples>> below)_
* The asterisk (`*`) can be used as a wildcard at the end of the metric name.
...
** `mp.metrics.distribution.timer.*` accepts an integer value with an appended time unit (valid time units are `ms` for milliseconds, `s` for seconds, `m` for minutes and `h` for hours). Decimal values will not be accepted. Values with no time unit will default to milliseconds. _(See <<min-max-sample,examples>> below)_
* The asterisk (`*`) can be used as a wildcard at the end of the metric name.
