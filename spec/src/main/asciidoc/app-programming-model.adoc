...
==== Annotated Naming Convention
Annotated metrics are registered into the _application_ `MetricRegistry` with the name based on

- The annotation's `name` parameter (herein referred to as "local name"),  
- the annotation's `absolute` parameter and
- the fully qualified (canonical) name of the declaring class.

If the `name` parameter is not specified, then the local name will be the short name of the declaring element (name of the field or method).
For constructors, the local name in this case is the simple name of the class to which this constructor belongs.
If the metric annotation is applied on class level and the `name` parameter is specified, then the value of the `name` parameter
replaces the fully qualified name of the declaring class for the purpose of computing the metric name for each field and method of the annotated class.

When the `absolute` parameter is `true`, then the resulting metric name is equal to just the local name.
When the `absolute` parameter is `false`, then the fully qualified name of the declaring class is prepended to it, separated by a dot.
