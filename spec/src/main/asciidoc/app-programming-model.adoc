...
It is expected in a micro services environment that a server hosts one application only.
This assumption is not always true.
MicroProfile Metrics supports multiple applications deployed in one server.
The default is the one application per server case.

MicroProfile metrics will keep per application registries when more than application is deployed.

The name of the application is determined by its `@javax.ws.rs.ApplicationPath` value.

The application programmer can deliver an explicit value via the `@MetricAppName` annotation
if the application is not a JAX-RS application or a different value is desired.
...
* If more than one application is deployed, the implementation must
** keep metrics of different applications deployed separate.
** throw an exception during deployment in case no application name is implicitly or explicitly given.
...
|@MetricAppName| Provides a name for the application | Value of `@javax.ws.rs.ApplicationPath` if provided. None otherwise
|===

==== Fields

All annotations (Except `RegistryType` and `MetricAppName`) have the following fields that correspond to the metadata fields described
in <<meta-data-def>>.
...
The implementation must register metrics for the current application if more than one application is deployed (see <<app-multi-app>> ).
Is must also filter for the current application when metrics are retrieved from the registry.

TIP: The implementor may chose to use tags to implement this separation and filtering.
